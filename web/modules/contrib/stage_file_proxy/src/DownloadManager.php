...
final class DownloadManager implements DownloadManagerInterface {
  ...
  public function fetch(string $server, string $remote_file_dir, string $relative_path, array $options): bool {
    ...
    try {
      ...
      // Prepare local target directory and save a downloaded file.
      ...
      $destination = str_replace('///', '//', "$destination/") . basename($relative_path);
  ...
  protected function fetchResponse(string $server, string $remote_file_dir, string $relative_path, array $options): ResponseInterface {
    try {
      ...
      if ($e->getCode() == 404) {
        // If this is a 404, it may be an image style converting the
        // file type.
  ...
  public function styleOriginalPath(string $uri, bool $style_only = TRUE) {
    ...
    // It is a style path, so we extract the different parts.
  ...
  /**
   * Use write and rename instead of write.
   *
  ...
  private function writeFile(string $destination, string $data): bool {
    ...
    // Save to the temporary filename in the destination directory.
    ...
    // Final check; make sure a file exists and is not empty.
    $result = FALSE;
